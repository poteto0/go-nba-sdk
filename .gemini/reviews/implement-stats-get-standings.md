# Review: Implement Stats.GetIstStandings (IST Standings)

## Summary

Implemented the `GetIstStandings` method (referencing `iststandings` endpoint) in the `Stats` namespace.
...
- **[Pass] Namespace API**: Added `GetIstStandings` to `IStatsNamespace` and `StatsNamespace`.
